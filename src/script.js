...
if (isNaN(P) || isNaN(N) || isNaN(R) || P <= 0 || N <= 0 || R <= 0) {
  // skip incomplete or invalid entries silently
  continue;
}
...
localStorage.setItem('emiwhiz_saved_data', JSON.stringify(data));
      ...
      loadSavedData() {
        ...
        // For this demo, we'll just initialize with default values
        if (this.savedData) {
          this.prepaymentSlider.value = this.savedData.prepayment;
          this.prepaymentValue.textContent = this.savedData.prepayment + '%';
          // Load loan data...
        }
      }
